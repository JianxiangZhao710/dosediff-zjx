#!/usr/bin/env python3
"""
dose_train_3d_v2_0.py
=====================
v2.0 (Dynamic Relation-to-Field Constraint Router) training script.

Upgrades v1.5's fixed "penalty field + fixed risk mask" injection to a
learnable constraint-field compiler + dynamic time/risk-aware router:

    penalty_basis -> ConstraintFieldCompiler3D -> compiled field
                  -> PenaltyAdapter3D -> ZeroConv (0-init)
                  -> DynamicConstraintRouter3D (per voxel / level / t)
                  -> X-stream injection

Backbone (v1.2), penalty adapter and ZeroConv "safe injection" are kept;
ZeroConv stays 0-init so the first forward is identical to v1.5 / v1.2.

Ablation modes (--mode):
    v1_5          : no compiler / no router (== v1.5 fixed W_risk).
    compiler_only : learnable compiled field, fixed W_risk injection.
    router_only   : raw basis, dynamic router injection.
    full_v2_0     : compiler + dynamic router (default).

Optional losses (default off / tiny for stability):
    --lambda_grad_loss   risk-region gradient-consistency loss.
    --lambda_hf_loss     high-frequency structure-preservation loss.
    --router_reg_weight  mean-injection penalty (anti global-injection).
    --router_smooth_weight router total-variation (smoothness) penalty.

Resume from a v1.5 / v1.4 / v1.2 checkpoint with --resume_from
(strict=False): backbone / adapter / zero-convs are reused; the new
compiler / relation encoder / routers start fresh (router init == v1.5).
"""
import sys
sys.path.append("../")
sys.path.append("./")
...
from guided_diffusion.dose_loader_3d_v2_0 import (
    Dataset_PSDM_3D_Train_v2_0,
    DEFAULT_V20_PENALTY_FILES,
)
...
# ----------------------------- CLI ARGS -----------------------------
parser = argparse.ArgumentParser(description="Train v2.0 (dynamic relation-to-field constraint router).")
...
# Penalty branch (inherited from v1.4/v1.5)
...
# v1.5 risk-aware prior (kept; now a router prior, not the final weight)
...
parser.add_argument("--router_time_independent", action='store_true',
                    help="disable timestep conditioning in the router")
parser.add_argument("--router_delta_scale", type=float, default=0.5)
parser.add_argument("--router_hidden", type=int, default=None)

# v2.0 optional losses / regularisers (default off / tiny)
parser.add_argument("--lambda_grad_loss", type=float, default=0.0)
parser.add_argument("--lambda_hf_loss", type=float, default=0.0)
parser.add_argument("--router_reg_weight", type=float, default=0.0)
parser.add_argument("--router_smooth_weight", type=float, default=0.0)
parser.add_argument("--disable_morphology_loss", action='store_true',
                    help="force lambda_grad_loss = lambda_hf_loss = 0")
parser.add_argument("--hf_kernel", type=int, default=5)

# Data paths
parser.add_argument("--data_root_train", type=str, default='/root/autodl-tmp/train-pats_preprocess/')
parser.add_argument("--data_root_val", type=str, default='/root/autodl-tmp/validation-pats_preprocess/')
...
# Resolve ablation mode + module switches.
...
if args.penalty_channels is not None:
    ...
    penalty_channels = 1 if args.penalty_mode == 'single' else len(DEFAULT_V20_PENALTY_FILES)
...
# ----------------------------- SAVE DIR -----------------------------
save_name = (
    f"MedSegDiff_Flow_3D_OpenKBP_v2_0_{mode}_mc{args.model_channels}"
...
if args.save_dir:
    ...
    save_dir = os.path.join('trained_models', 'v2_0_dynamic_router', save_name)
if is_main:
...
# ----------------------------- DATA -----------------------------
train_data = Dataset_PSDM_3D_Train_v2_0(
...
if val_enabled:
    val_data = Dataset_PSDM_3D_Train_v2_0(
...
if is_main:
    ...
    if val_enabled:
        print(f"Val   size: {len(val_data)}  (val_every={args.val_every}, val_steps={args.val_steps})", flush=True)
    else:
        print("Val disabled", flush=True)
...
if is_main:
    ...
    print(
        f"[Model] UNetModel_DynamicConstraintRouter_v2_0 | mode={mode} | "
        f"total={n_params_total/1e6:.2f}M | adapter+compiler+routers={n_params_adapter/1e6:.2f}M",
        flush=True,
    )
    print(
        f"[v2.0] compiler={use_compiler} router={use_router} relation={use_relation} "
        f"time_dep={not args.router_time_independent} delta_scale={args.router_delta_scale} | "
        f"risk_base={args.risk_base} risk_use={args.risk_use_channels}",
        flush=True,
    )
    print(
        f"[v2.0] lambda_grad={args.lambda_grad_loss} lambda_hf={args.lambda_hf_loss} "
        f"router_reg={args.router_reg_weight} router_smooth={args.router_smooth_weight}",
        flush=True,
    )

# ----- Resume model (strict=False so v1.2/v1.4/v1.5 ckpts load cleanly) -----
if args.resume_from:
    ...
    if is_main:
        n_new = sum(
            1 for k in missing
            if k.startswith('constraint_compiler.')
            or k.startswith('routers.')
            or k.startswith('router_middle.')
        )
        print(
            f"[Resume] missing={len(missing)} (v2.0 new modules: {n_new}); unexpected={len(unexpected)}",
            flush=True,
        )
    del state_dict

# ----- Adapter warm-up: freeze the v1.2 backbone -----
if args.adapter_warmup_epochs > 0 and start_epoch < args.adapter_warmup_epochs:
    model.freeze_backbone()
    backbone_frozen = True
    if is_main:
        n_trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
        print(f"[Warmup] Backbone frozen — only training adapter+compiler+routers "
              f"(trainable={n_trainable/1e6:.2f}M, for {args.adapter_warmup_epochs} epochs).", flush=True)
else:
...
# ----------------------------- VAL FUNCTION -----------------------------
@torch.no_grad()
def evaluate_metrics(use_ema: bool):
    """Return (weighted body MAE, risk-region MAE) in Gy."""
    if not val_enabled:
    ...
    risk_mae_sum = torch.zeros(1, device=device)
    risk_mae_count = torch.zeros(1, device=device)
    ...
    with cm:
        ...
        for ct, syn_cond, dis_cond, penalty, dose_gt in iterator:
            ...
            # Risk-region mask from the base risk map exposed by the UNet.
            ...
            for b in range(ct.shape[0]):
                ...
                if risk_map is not None:
                    ...
                    rm_sum = rm.sum()
                    if rm_sum.item() > 0:
                        risk_mae_b = (torch.abs(pred_gy[b] - gt_gy[b]) * rm).sum() / rm_sum
                        risk_mae_sum += risk_mae_b
                        risk_mae_count += 1

    for t in (mae_sum, mae_count, risk_mae_sum, risk_mae_count):
        dist.all_reduce(t, op=dist.ReduceOp.SUM)
    ddp_model.train()

    mae = (mae_sum / mae_count).item() if mae_count.item() > 0 else float('inf')
    risk_mae = (risk_mae_sum / risk_mae_count).item() if risk_mae_count.item() > 0 else float('nan')
    return mae, risk_mae
...
for epoch in range(start_epoch, total_epochs):
    if backbone_frozen and epoch >= args.adapter_warmup_epochs:
        ...
        if is_main:
            print(f"[Warmup] Done. Unfroze backbone at epoch {epoch + 1}.", flush=True)
    ...
    for i, batch in enumerate(loader):
        ...
        if is_accum_step or is_last_batch:
            torch.nn.utils.clip_grad_norm_(
                [p for p in ddp_model.parameters() if p.requires_grad], args.grad_clip,
            )
            optimizer.step()
            optimizer.zero_grad(set_to_none=True)
            if ema is not None:
    ...
    if run_val:
        val_mae, val_risk_mae = evaluate_metrics(use_ema=(ema is not None))
        if is_main:
            tag = "EMA" if ema is not None else "raw"
            print(f"  [Val] epoch {epoch + 1}  MAE({tag})={val_mae:.4f} Gy  "
                  f"risk-MAE={val_risk_mae:.4f} Gy  (best={best_mae:.4f} @epoch{best_mae_epoch})", flush=True)
            if val_mae < best_mae:
                best_mae = val_mae
