#!/usr/bin/env python3
"""
dose_predict_3d_v1_5.py
=======================
v1.5 (Risk-aware ControlNet-lite penalty adapter) full-volume
sliding-window inference. Same interface as ``dose_predict_3d_v1_4.py``;
the only differences are the v1.5 model class, the v1.5 default penalty
files / channel order, and the risk-aware configuration.

Penalty I/O:
    --penalty_mode single  -> penalty == dose_synthetic.nii.gz (P=1, == P_fused).
    --penalty_mode multi   -> [S_target, A_oar, B_boundary, P_fused] mapped to
                              S_target.nii.gz / A_oar.nii.gz / B_boundary.nii.gz /
                              dose_synthetic.nii.gz (overridable via --penalty_files).

Penalty dropout AND the risk mask are computed identically to training,
except dropout is automatically disabled at inference (model.eval()).
The risk-aware modulation itself stays active so high-risk regions
receive stronger penalty guidance.
"""
import sys
import gc
...
def main():
    parser = argparse.ArgumentParser(description='v1.5 Risk-aware ControlNet-lite penalty adapter inference')
    ...
    # Resolve penalty source + channel names.
    if args.penalty_mode == 'single':
        penalty_files = ()  # penalty == dose_synthetic
        penalty_channels = 1
        penalty_channel_names = args.penalty_channel_names or ['P_fused']
        print("[v1.5] Penalty mode=single (penalty == dose_synthetic.nii.gz == P_fused)")
    else:
        ...
        print(f"[v1.5] Penalty mode=multi files={list(penalty_files)} channels={penalty_channels} "
              f"names={penalty_channel_names}")

    use_risk = (not args.disable_risk_aware_injection)
    print(f"[v1.5] risk_aware={use_risk} risk_base={args.risk_base} "
    ...
    model = UNetModel_RiskAwarePenaltyAdapter_v1_5(
        ...
        dims=3,
        # Inference: penalty dropout force-disabled.
