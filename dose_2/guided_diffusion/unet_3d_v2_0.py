...
class UNetModel_DynamicConstraintRouter_v2_0(UNetModel_RiskAwarePenaltyAdapter_v1_5):
    ...
    # ------------------------------------------------------------------ #
    # Gradient-checkpointed v2.0 stages (compiler / adapter / router)
    # ------------------------------------------------------------------ #
    def _forward_compiler_ckpt(self, penalty, dis, cond):
        if cond is None:
            def _run(penalty_, dis_):
                return self.constraint_compiler(penalty_, dis=dis_, cond=None)

            inputs = (penalty, dis)
        else:
            def _run(penalty_, dis_, cond_):
                return self.constraint_compiler(penalty_, dis=dis_, cond=cond_)

            inputs = (penalty, dis, cond)

        return checkpoint(
            _run,
            inputs,
            list(self.constraint_compiler.parameters()),
            self.use_checkpoint,
        )

    def _forward_penalty_adapter_ckpt(self, adapter_input):
        def _run(inp):
            return tuple(self.penalty_adapter(inp))

        out = checkpoint(
            _run,
            (adapter_input,),
            list(self.penalty_adapter.parameters()),
            self.use_checkpoint,
        )
        return list(out) if isinstance(out, tuple) else [out]

    def _forward_penalty_injection_ckpt(
        self, h_x, penalty_feat, m_risk, w_floor, emb, level_idx, zero_conv,
    ):
        router_mod = self.routers[level_idx] if self.use_router and self.routers is not None else None

        def _run(h_x_, pf_, m_risk_, w_floor_, emb_):
            residual = zero_conv(pf_)
            if router_mod is not None:
                risk_l = self._resize_risk(m_risk_, residual)
                w_l = router_mod(h_x_, pf_, risk_l, emb_)
            else:
                w_l = self._resize_risk(w_floor_, residual)
            return h_x_ + w_l * residual, w_l

        inject_params = list(zero_conv.parameters())
        if router_mod is not None:
            inject_params += list(router_mod.parameters())

        return checkpoint(
            _run,
            (h_x, penalty_feat, m_risk, w_floor, emb),
            inject_params,
            self.use_checkpoint,
        )

    def _forward_bottleneck_injection_ckpt(
        self, h, penalty_feat, m_risk, w_floor, emb, zero_conv, router_mod,
    ):
        def _run(h_, pf_, m_risk_, w_floor_, emb_):
            residual = zero_conv(pf_)
            if router_mod is not None:
                risk_mid = self._resize_risk(m_risk_, residual)
                w_mid = router_mod(h_, pf_, risk_mid, emb_)
            else:
                w_mid = self._resize_risk(w_floor_, residual)
            return h_ + w_mid * residual, w_mid
        ...
        return checkpoint(
            _run,
            (h, penalty_feat, m_risk, w_floor, emb),
            inject_params,
            self.use_checkpoint,
        )
    ...
    def forward(self, x, timesteps, ct, syn, dis, penalty, y=None):
        ...
        if self.use_compiler and self.constraint_compiler is not None:
            ...
            adapter_input = self._forward_compiler_ckpt(
                penalty.type(self.dtype), dis.type(self.dtype), cond,
            )
            if self.collect_aux:
        ...
        penalty_feats = self._forward_penalty_adapter_ckpt(adapter_input)
        ...
        for i, module in enumerate(self.input_blocks):
            ...
            h_x, w_l = self._forward_penalty_injection_ckpt(
                h_x, penalty_feats[level], m_risk, w_floor, emb, i, zero_conv_i,
            )
        ...
        router_mid = self.router_middle if self.use_router else None
        h, w_mid = self._forward_bottleneck_injection_ckpt(
            h, penalty_feats[-1], m_risk, w_floor, emb,
            self.penalty_zero_conv_middle, router_mid,
        )
