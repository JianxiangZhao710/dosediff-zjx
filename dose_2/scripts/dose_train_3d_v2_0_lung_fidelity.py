#!/usr/bin/env python3
"""
dose_train_3d_v2_0_lung_fidelity.py
===================================
From-scratch v2.0 lung dose training with a **fidelity-first** loss
(``FlowMatchingFidelity``): prioritise voxel-wise match to the ground-truth
label (Dose Score / MAE) instead of the DVH-oriented structure weighting.

Loss:
    L = base velocity-MSE (flattened weights ptv/oar/bg, default 2/1.5/1)
      + lambda_recon_l1 * L1(pred_clean, gt, body)      # main fidelity
      + lambda_recon_l2 * L2(pred_clean, gt, body)
      + lambda_grad_loss * grad(full image)             # anti over-smoothing
      + lambda_hf_loss   * hf(full image)

This is a SEPARATE script; it does not modify any existing file. It reuses
the lung dataset, the v2.0 UNet, and adds the fidelity wrapper. Trains from
scratch by default (no resume).

Example
-------
    cd /root/dose-zjx/dose_2
    python scripts/dose_train_3d_v2_0_lung_fidelity.py --gpu 0 --epoch 400
or
...
from flow_matching_fidelity import FlowMatchingFidelity


# ----------------------------- CLI ARGS -----------------------------
def build_args():
    p = argparse.ArgumentParser(description="From-scratch fidelity-first v2.0 lung training.")
    ...
    # ---- fidelity-first loss ----
    p.add_argument("--ptv_weight", type=float, default=2.0)
    p.add_argument("--oar_weight", type=float, default=1.5)
    p.add_argument("--bg_weight", type=float, default=1.0)
    p.add_argument("--lambda_recon_l1", type=float, default=1.0)
    p.add_argument("--lambda_recon_l2", type=float, default=0.2)
    p.add_argument("--recon_whole_volume", action="store_true",
                   help="reconstruct over the whole patch instead of the CT body mask")
    p.add_argument("--lambda_grad_loss", type=float, default=0.05)
    p.add_argument("--lambda_hf_loss", type=float, default=0.05)
    p.add_argument("--hf_kernel", type=int, default=5)
    p.add_argument("--morphology_risk_weighted", action="store_true",
                   help="restrict grad/HF losses to high-risk regions (default: full image)")
...
def main():
    ...
    # ----------------------------- SAVE DIR -----------------------------
    save_name = (
        f"v2_0_lung_fidelity_{mode}_mc{args.model_channels}_pc{penalty_channels}"
        f"_bs{args.bs}_epoch{args.epoch}"
    )
    if args.save_name_suffix:
        save_name += "_" + args.save_name_suffix
    save_dir = args.save_dir or os.path.join("trained_models", "v2_0_lung_fidelity", save_name)
    ...
    print(f"[Model] v2.0 fidelity mode={mode} | total={n_total/1e6:.2f}M")
    print(f"[Loss] base weights ptv/oar/bg={args.ptv_weight}/{args.oar_weight}/{args.bg_weight} | "
          f"recon_l1={args.lambda_recon_l1} recon_l2={args.lambda_recon_l2} "
          f"(body_mask={not args.recon_whole_volume}) | "
          f"grad={args.lambda_grad_loss} hf={args.lambda_hf_loss} "
          f"risk_weighted={args.morphology_risk_weighted}")

    # ----- optional resume (default: from scratch) -----
    ...
    if args.resume_from:
        ...
        print("[Resume] none -> training FROM SCRATCH")

    flow_model = FlowMatchingFidelity(
        model,
        ptv_weight=args.ptv_weight,
        oar_weight=args.oar_weight,
        bg_weight=args.bg_weight,
        lambda_recon_l1=args.lambda_recon_l1,
        lambda_recon_l2=args.lambda_recon_l2,
        recon_use_body_mask=(not args.recon_whole_volume),
        lambda_grad_loss=args.lambda_grad_loss,
        lambda_hf_loss=args.lambda_hf_loss,
        hf_kernel=args.hf_kernel,
        morphology_risk_weighted=args.morphology_risk_weighted,
    ...
    for epoch in epoch_bar:
        ...
        for i, batch in enumerate(batch_bar):
            ...
            for k in ("base", "recon_l1", "recon_l2", "grad", "hf"):
