...
# ----------------------------- CLI ARGS -----------------------------
def build_args():
    p = argparse.ArgumentParser(description="Single-GPU v2.2 lung dose training.")
    ...
    # v2.2 modules / ablation (same flags as v2.0 + full_v2_2)
    ...
    p.add_argument("--freeze_backbone_epochs", type=int, default=0,
                   help="freeze UNet backbone for first N epochs (train compiler+adapter+router)")
...
def main():
    ...
    print(f"[Model] v2.2 mode={mode} | total={n_total/1e6:.2f}M | "
    ...
    print(f"[v2.2] compiler=v22(gate={args.compiler_gate_mode}) router={use_router} "
          ...
          f"reset_compiler={args.reset_compiler}")
    ...
    if args.resume_from:
        ...
        load_warmstart_checkpoint(model, args.resume_from, reset_compiler=args.reset_compiler)
    ...
    if ema is not None and args.resume_ema and os.path.isfile(args.resume_ema) and not args.reset_compiler:
        ...
        del es
    elif ema is not None and args.reset_compiler:
        print("[Resume] reset_compiler=True -> EMA shadow uses fresh compiler weights")
    ...
    for epoch in epoch_bar:
        ...
        if epoch == args.freeze_backbone_epochs and args.freeze_backbone_epochs > 0:
            epoch_bar.write(f"[Freeze] backbone grad enabled at epoch {epoch + 1}")
        model.train()
        ...
        for i, batch in enumerate(batch_bar):
            ...
            if (i + 1) % grad_accum == 0 or (i + 1) == n_batches:
                clip_params = (
                    ...
                    if backbone_frozen
                    else [p for p in model.parameters() if p.grad is not None]
                )
