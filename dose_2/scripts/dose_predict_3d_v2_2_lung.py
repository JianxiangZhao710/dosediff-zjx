...
-------
    ...
    python scripts/dose_predict_3d_v2_2_lung.py \
...
def main():
    p = argparse.ArgumentParser(description="v2.2 lung dose inference.")
    ...
    print(f"[v2.2-lung] mode={mode} gate={args.compiler_gate_mode} compiler={use_compiler} "
          f"router={use_router} relation={use_relation} penalty={list(penalty_files)}")
    ...
    torch.cuda.empty_cache()
    flow_model = FlowMatchingV22(model)
