#!/usr/bin/env python3
"""
dose_train_3d_v2_0_lung_fidelity_ddp.py
=======================================
2-GPU (DDP) from-scratch fidelity-first v2.0 lung training.

Same objective as ``dose_train_3d_v2_0_lung_fidelity.py`` (``FlowMatchingFidelity``),
but uses ``torch.distributed`` + ``DistributedDataParallel``.

Launch (2 GPUs):
    cd /root/dose-zjx/dose_2
    torchrun --nproc_per_node=2 scripts/dose_train_3d_v2_0_lung_fidelity_ddp.py ...
or
...
def build_args():
    p = argparse.ArgumentParser(description="DDP fidelity-first v2.0 lung training.")
...
if is_main:
    ...
    print(f"[Model] v2.0 fidelity DDP mode={mode} | total={n_total/1e6:.2f}M", flush=True)
    print(f"[Loss] ptv/oar/bg={args.ptv_weight}/{args.oar_weight}/{args.bg_weight} | "
          f"recon_l1={args.lambda_recon_l1} recon_l2={args.lambda_recon_l2} | "
          f"grad={args.lambda_grad_loss} hf={args.lambda_hf_loss}", flush=True)

if args.resume_from:
    if not os.path.isfile(args.resume_from):
        raise FileNotFoundError(f"--resume_from not found: {args.resume_from}")
    if is_main:
        print(f"[Resume] from {args.resume_from} (strict=False)", flush=True)
