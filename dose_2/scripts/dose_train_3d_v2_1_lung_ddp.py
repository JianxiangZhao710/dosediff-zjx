#!/usr/bin/env python3
"""
dose_train_3d_v2_1_lung_ddp.py
================================
2-GPU (DDP) v2.1 lung training (compiler: no P_fused shortcut + softplus pos/neg).

Same loss as v2.0 (``FlowMatchingV21`` / structure-weighted MSE). No new loss terms.

Launch (2 GPUs):
    cd /root/dose-zjx/dose_2
    torchrun --nproc_per_node=2 scripts/dose_train_3d_v2_1_lung_ddp.py ...
or
...
from guided_diffusion.unet_3d_v2_1 import (
    UNetModel_DynamicConstraintRouter_v2_1,
    resolve_v2_1_mode,
    V2_1_MODES,
)
from guided_diffusion.unet_3d_v1_5 import DEFAULT_RISK_USE_CHANNELS
from flow_matching_v2_1 import FlowMatchingV21


def build_args():
    p = argparse.ArgumentParser(description="DDP v2.1 lung training (2-GPU).")
    ...
    p.add_argument("--mode", type=str, choices=list(V2_1_MODES), default="full_v2_1")
...
mode, use_compiler, use_router = resolve_v2_1_mode(
    args.mode,
...
save_name = (
    f"v2_1_lung_{mode}_mc{args.model_channels}_pc{penalty_channels}"
    f"_bs{world_size * args.bs}_epoch{total_epochs}"
)
if args.save_name_suffix:
    save_name += "_" + args.save_name_suffix
save_dir = args.save_dir or os.path.join("trained_models", "v2_1_lung", save_name)
if is_main:
...
model = UNetModel_DynamicConstraintRouter_v2_1(
...
if is_main:
    ...
    print(f"[Model] v2.1 DDP mode={mode} | total={n_total/1e6:.2f}M", flush=True)
    print(f"[v2.1] compiler=v21(no_p_bypass+softplus) router={use_router} relation={use_relation}", flush=True)
